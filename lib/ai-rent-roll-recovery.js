...
export async function recoverRentRollWithAI({ text, tables, filename, jobId: _jobId, includeDiagnostics = false }) {
  ...
  if (!promptText && !promptTables) return finish(null, 'candidate_missing', { rejection_reason: 'missing_source_text_or_tables' });
  ...
  try {
    ...
    return finish(validated, 'accepted', { validation_accepted: true });
  } catch (_err) {

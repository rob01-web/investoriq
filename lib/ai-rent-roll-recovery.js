...
const OPENAI_TIMEOUT_MS = Math.min(
  55000,
  Math.max(10000, Number(process.env.OPENAI_RENT_ROLL_RECOVERY_TIMEOUT_MS) || 55000)
);
...
const buildUnitMix = (units) => {
  ...
  return Object.entries(mixMap).map(([unit_type, value]) => ({
    unit_type,
    count: value.count,
    current_rent_count: value.current.length,
    market_rent_count: value.market.length,
    current_rent:
      value.current.length > 0
        ? value.current.reduce((sum, amount) => sum + amount, 0) / value.current.length
        : null,
    market_rent:
      value.market.length > 0
        ? value.market.reduce((sum, amount) => sum + amount, 0) / value.market.length
        : null,
  }));
};
...
export async function recoverRentRollWithAI({ text, tables, filename, jobId: _jobId, includeDiagnostics = false }) {
  const diagnostics = {
    attempted: true,
  ...
  if (!promptText && !promptTables) {
    return finish(null, 'candidate_missing', {
      rejection_reason: 'missing_source_text_or_tables',
      rejection_reasons: ['missing_source_text_or_tables'],
    });
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const provider = classifyOpenAiError(response.status, errorBody, null, response.headers);
    ...
    const validated = validateAiRentRollRecoveryPayload(candidate);
    if (!validated) {
      const rejectionReasons = [];
      const confidence = parseNumberLike(candidate?.confidence);
      if (candidate?.is_rent_roll !== true) rejectionReasons.push('candidate_not_rent_roll');
      if (!Number.isFinite(confidence) || confidence < MIN_CONFIDENCE) rejectionReasons.push('candidate_below_confidence');

      const summary = normalizeSummary(candidate?.summary);
      const units = normalizeUnits(candidate?.units);
      if (units.length === 0) rejectionReasons.push('missing_unit_rows');
      if (!Number.isFinite(summary.total_units)) rejectionReasons.push('missing_total_units');
      if (!Number.isFinite(summary.occupied_units)) rejectionReasons.push('missing_occupied_units');
      if (!Number.isFinite(summary.occupancy)) rejectionReasons.push('missing_occupancy');
      if (
        !Number.isFinite(summary.monthly_in_place_rent) &&
        !Number.isFinite(summary.annual_in_place_rent) &&
        !units.some((row) => Number.isFinite(row?.in_place_rent))
      ) {
        rejectionReasons.push('missing_in_place_rent');
      }
      if (Number.isFinite(summary.total_units) && (summary.total_units <= 0 || summary.total_units > MAX_TOTAL_UNITS)) {
        rejectionReasons.push('invalid_unit_count');
      }
      if (Number.isFinite(summary.occupied_units) && summary.occupied_units < 0) {
        rejectionReasons.push('invalid_occupied_unit_count');
      }
      if (Number.isFinite(summary.occupancy) && (summary.occupancy < 0 || summary.occupancy > 1)) {
        rejectionReasons.push('invalid_occupancy');
      }
      if (
        (Number.isFinite(summary.monthly_in_place_rent) &&
          (summary.monthly_in_place_rent < 0 || summary.monthly_in_place_rent > MAX_TOTAL_MONTHLY_RENT)) ||
        (Number.isFinite(summary.annual_in_place_rent) &&
          (summary.annual_in_place_rent < 0 || summary.annual_in_place_rent > MAX_TOTAL_ANNUAL_RENT))
      ) {
        rejectionReasons.push('invalid_in_place_rent');
      }
      if (
        units.length === 0 &&
        !Number.isFinite(summary.total_units) &&
        ...
        rejectionReasons.push('vague_rent_roll_no_reliable_totals');
      }
      return finish(null, 'validation_rejected', {
        validation_accepted: false,
        rejection_reason: 'ai_rent_roll_candidate_validation_rejected',
        rejection_reasons: [
    ...
    return finish(validated, 'accepted', {
      ...
      validation_reasons: ['rent_roll_recovered_from_ai_validated'],
      accepted_fields: [
        'units',
        'unit_mix',
        'total_units',
        ...(validated.totals ? ['totals'] : []),
        ...(Number.isFinite(validated.occupancy) ? ['occupancy'] : []),
      ],
      derived_fields: [],
      field_diagnostics: [],
    });

...
import { evaluateFinancingSemanticEvidence } from "../api/_lib/support-doc-semantic-evidence.js";
...
const numericValuesInText = (value) => {
  const values = [];
...
const evidenceMatchesNumericCandidate = (candidateValue, excerpt) => {
  if (!Number.isFinite(candidateValue)) return false;
  return numericValuesInText(excerpt).some((sourceValue) => {
...
export function shouldAttemptAcquisitionPurchaseAssumptionsRecovery(text) {
  ...
  if (!semanticEvidence.hasAffirmativeAcquisitionEvidence) return false;
...
export function shouldAttemptCurrentMortgageRecovery(text) {
  ...
  const semanticEvidence = evaluateFinancingSemanticEvidence(source);
  if (!semanticEvidence.hasAffirmativeCurrentDebtEvidence) return false;
  if (semanticEvidence.hasAffirmativeAcquisitionEvidence && semanticEvidence.acquisitionScore > semanticEvidence.currentDebtScore) return false;
...
const validateFieldEvidence = (fieldName, candidateValue, sourceText, evidenceObject) => {
  ...
  const acceptedEvidence = evidence.find(
    (excerpt) => evidenceMatchesSource(excerpt, sourceText) && evidenceMatchesNumericCandidate(candidateValue, excerpt)
  ) || null;
...
const validateStringEvidence = (fieldName, candidateValue, sourceText, evidenceObject) => {
  ...
  const evidence = normalizeEvidenceArray(evidenceObject?.[fieldName]);
  const acceptedEvidence = evidence.find(
    (excerpt) => evidenceMatchesSource(excerpt, sourceText) && normalizeComparableText(excerpt).includes(normalizeComparableText(text))
  ) || null;
  if (!acceptedEvidence) return null;
  return {
    value: text,
...
const validateEvidenceString = (fieldName, candidateValue, sourceText, evidenceObject) => {
  ...
  const evidence = normalizeEvidenceArray(evidenceObject?.[fieldName]);
  const acceptedEvidence = evidence.find(
    (excerpt) => evidenceMatchesSource(excerpt, sourceText) && normalizeComparableText(excerpt).includes(normalizeComparableText(value))
  ) || null;
  if (!acceptedEvidence) return null;
...
export function validateAcquisitionPurchaseAssumptionsCandidateWithDiagnostics(candidate, sourceText) {
  ...
  const payload = {
    method: 'ai_support_doc_candidate_evidence_checked',
    ai_assisted: true,
    candidate_claim_status: 'evidence_checked',
    candidate_only: true,
    candidate_span_validated: true,
    candidate_extractor: 'ai_support_doc_recovery',
    confidence,

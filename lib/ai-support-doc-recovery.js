...
export function validateAppraisalCandidate(candidate, sourceText) {
  if (!candidate || candidate.is_appraisal_support !== true) return null;
  ...
  if (valuationDateCandidate && !valuationDateEvidence) parseWarnings.push('unverified_valuation_date');
  if (Number.isFinite(capRateCandidate) && !capRateEvidence) parseWarnings.push('unverified_cap_rate');
  if (Number.isFinite(egiCandidate) && !egiEvidence) parseWarnings.push('unverified_effective_gross_income');
  if (Number.isFinite(noiCandidate) && !noiEvidence) parseWarnings.push('unverified_noi');
  if (valueBasisCandidate && !valueBasisEvidence) parseWarnings.push('unverified_value_basis');
  if (appraisalTypeCandidate && !appraisalTypeEvidence) parseWarnings.push('unverified_appraisal_type');
...
const recoverSupportDocWithAI = async ({
  ...
  try {
    ...
    const renovationValidation =
      supportDocRecoveryType === 'renovation'
        ? validateRenovationCandidateWithDiagnostics(candidate, promptText)
        : null;
    const validated =
      supportDocRecoveryType === 'renovation' ? renovationValidation?.payload : validateCandidate(candidate, promptText);
    if (!validated) {
      const rejectionReasons =
        supportDocRecoveryType === 'renovation'
          ? renovationValidation?.diagnostics?.validation_reasons || ['deterministic_validation_rejected_candidate']
      ...
      return finish(null, 'validation_rejected', {
        ...
        row_diagnostics:
          supportDocRecoveryType === 'renovation' ? renovationValidation?.diagnostics?.row_diagnostics || [] : [],
        derived_fields:
          supportDocRecoveryType === 'renovation' ? renovationValidation?.diagnostics?.derived_fields || [] : [],
      });
    }
    ...
    return finish(validated, 'accepted', {
      ...
      validation_reasons:
        supportDocRecoveryType === 'renovation'
          ? renovationValidation?.diagnostics?.validation_reasons || []
          : [],
      row_diagnostics:
        supportDocRecoveryType === 'renovation' ? renovationValidation?.diagnostics?.row_diagnostics || [] : [],
      derived_fields:

...
function coverageSummary(coverage = {}) {
  if (!coverage || coverage.total === 0) return "Report sections follow available evidence.";
  const parts = [];
  if (coverage.included > 0) parts.push(`${coverage.included} full`);
  if (coverage.qualified > 0) parts.push(`${coverage.qualified} qualified`);
  if (coverage.compact > 0) parts.push(`${coverage.compact} concise`);
  if (coverage.collapsed > 0) parts.push(`${coverage.collapsed} not expanded`);
  if (coverage.omitted > 0) parts.push(`${coverage.omitted} not shown`);
  return parts.length > 0 ? parts.join(" | ") : "Report sections follow available evidence.";
}

function manifestPair(left, right) {
  return `<table class="iq-manifest-pair" role="presentation"><tbody><tr><td class="iq-manifest-column">${left}</td><td class="iq-manifest-column">${right}</td></tr></tbody></table>`;
}
...
export function renderFullUnderwritingQualityManifestV1Html(contract = null) {
  ...
  const identityCard = `<div class="card no-break">
  ...
  const evidenceCard = `<div class="card no-break">
  ...
  const coverageCard = `<div class="card no-break">
  ...
  const calculationCard = `<div class="card no-break">
  ...
  return `<section class="section" id="quality-manifest-title" data-iq-section="quality-manifest" style="page-break-before:always;break-before:page;">
    <style>
      ...
      #quality-manifest-title .iq-manifest-pair > tbody > tr > td:last-child { padding-left:10px; }
      #quality-manifest-title .iq-manifest-column .card { width:100%; min-width:0; overflow-wrap:anywhere; word-break:break-word; }
      #quality-manifest-title .meta-line { min-width:0; }
      #quality-manifest-title .meta-value { min-width:0; overflow-wrap:anywhere; word-break:break-word; }
      @media print {
        #quality-manifest-title .iq-manifest-pair { break-inside:avoid-page; page-break-inside:avoid; }
        #quality-manifest-title .iq-manifest-pair > tbody > tr { break-inside:avoid-page; page-break-inside:avoid; }
      }
    </style>
    <div class="section-header"><span class="section-header-title">Quality Manifest</span></div>
    <p class="body-copy">This manifest summarizes the evidence basis, analytical coverage, calculation boundaries, and report traceability for this underwriting report.</p>
    ${manifestPair(identityCard, evidenceCard)}
    ${manifestPair(coverageCard, calculationCard)}

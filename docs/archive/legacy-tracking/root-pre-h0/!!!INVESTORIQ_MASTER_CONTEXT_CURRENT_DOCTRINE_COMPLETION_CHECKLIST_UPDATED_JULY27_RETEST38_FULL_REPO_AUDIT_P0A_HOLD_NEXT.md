...
RENOVATION ROW FIELD-LEVEL PROVENANCE: PRESERVED / 35797d8
UNLABELED ADJACENT MARKET AMOUNTS: REJECTED
PHASE 3 DETERMINISTIC RECEIPT MAP AND CALCULATIONS: PASS
PHASE 4 VALIDATED PREMIUM MODEL: PASS
PHASE 5 GUARDED RENDERER INSERTION: PASS / INTERNAL-TEST ONLY
PHASE 6 OBSERVE-ONLY COMPLETENESS: PASS
PHASE 7 INTERNAL PDF COMPOSITION: PASS / 16 PAGES / PDF BOSS CERTIFIED
INTERNAL PREMIUM CERTIFICATION: PASS / INTERNAL-TEST ONLY
INTERNAL JOB-SURFACE AUTHORITY: PASS / INTERNAL-TEST ONLY
FLAG-OFF BASE OUTPUT EQUIVALENCE: PASS
PREMIUM DELIVERY OR PUBLICATION AUTHORITY: NONE
WORKER INTEGRATION: NOT STARTED
EXTERNAL PREMIUM ENFORCEMENT: NOT STARTED
EXTERNAL FEATURE ACTIVATION: FALSE
DEPLOYED: NO
LIVE RETEST: NOT RUN
NEXT: Separately bounded Phase 8 worker/job-start surface integration, followed by separately reviewed external premium certification and enforcement
```
...
# July 27, 2026 Controlling Addendum — RETEST 37/38 Closure, Premium Flag-Off, Report History Repair, and Claude Opus Full-Repository Launch Investigation

This addendum is the controlling continuation point. Earlier checkpoints remain historical evidence only.
...
RETEST 37:
HOLD / failed canary.
Canonical Source Truth and core publication eligibility passed.
Final Premium enforcement could not load the immutable job-start surface receipt.
The failed canary was rolled back and the receipt persistence/reload boundary was repaired.
Credit restoration succeeded.

RETEST 38:
HOLD / failed canary.
Canonical Premium assignment succeeded.
Source Truth and deterministic calculations passed.
Delivery Gate was deliverable.
A real 19-page production PDF rendered.
PDF Publication Quality Boss detected approved-surface extraction loss.
Bounded automatic recomposition was incorrectly skipped because
PDF_TABLE_CONTINUATION_HEADER_MISSING was absent from recovery eligibility.
...
## Deployed repair checkpoint

```text
8545d69
RETEST 38's complete recoverable PDF incident set now enters bounded
recomposition and recertification.

2544969
Removed the separate top Reload button.
Report History Refresh now updates completed, active, failed,
entitlement, and credit-remedy state.
Failed reports appear directly in Report History.

33dac6f
Doctrine and operational receipts updated.
```

No PDF Publication Quality Boss rule was weakened. A recomposed PDF must still pass complete recertification before publication.

Production deployment receipt:

```text
dpl_A7DCmhqyNVu8VJmUG6uxhcDdbPt1
Ready / aliased / HTTP 200
```

Premium assignment remains exactly:

```text
false
```

Working tree was reported clean after deployment.

## Browser verification completed by Rob

Rob manually verified the deployed customer dashboard:

```text
Top Reload button: GONE
RETEST 38 in Report History: PRESENT
RETEST 38 state: FAILED
Restored-credit customer messaging: PRESENT
```

This closes the known hidden-failure-history UI defect. Report History is now the canonical customer location for active, completed, and failed report state.

## Publish-or-Collapse constitutional correction
...
-> bounded repair and recertification
-> publish with affected optional surface collapsed, omitted, or qualified
-> publish safe base report with explicit limitations and remedy
-> complete failure only when no materially correct report can exist
```

The only ordinary document-quality blocker families remain:
...
Optional support, Premium-only analysis, AI recovery, layout, pagination, continuation-header, chart, table, and non-core certification defects must not automatically become complete customer failure when a materially correct base report can still exist.

## Claude Opus full-repository investigation

A repository-wide investigation was completed on the isolated branch:

```text
investigation/full-repo-underwriting-audit
```

No audit work was merged to `main`, and no production implementation was performed from the audit branch.

Durable investigation records were created in:
...
The audit receipt reported:

```text
193 repository records inspected
151 QA files inventoried
76 proven findings
10 launch blockers
84 doctrine/archive files classified
6 contradictory doctrine families
12 archived-but-reachable semantic families
```
...
Shortest safe launch path after P0 repairs.

FULL UNDERWRITING:
Not launchable this week under the current contract.

RECOMMENDED EVENTUAL FULL UNDERWRITING LANE:
Existing V2/base Underwriting lane.

PREMIUM ACQUISITION UNDERWRITING:
Remain feature-flagged off.
Do not use as the launch lane yet.

REWRITE REQUIRED:
No.
The repository is salvageable through bounded authority and state-machine repairs.
```

The old legacy Underwriting pipeline remains evidence of the original commercial vision, not a source of factual authority. Safe presentation concepts may be recovered only after rebinding them to canonical Source Truth and deterministic calculations. Unsafe legacy factual fallbacks remain retired.

## Proven architectural failure themes

The audit identified recurring systemic causes rather than one isolated renderer defect:

```text
split worker claimers
missing leases and fencing
non-atomic claim, entitlement, receipt, and restoration transitions
multiple surface and identity authorities
shared V1/V2 identity
active legacy factual fallbacks
producer/renderer schema divergence
missing Premium-to-safe-base fallback
customer-surface re-derivation
competing rent and calculation authorities
parser and classification inconsistency
filename-based gating
duplicate-source competition
unbounded AI recovery and internal fetches
fragmented frontend/report-history state
contradictory doctrine and reachable semantic archives
insufficient end-to-end constitutional contract proof
```

These interacting defects explain the repeated whack-a-mole pattern: each canary can pass upstream Source Truth while failing later at a different independent authority or state transition.

## Preliminary launch lane decision

The current protected direction is:

```text
Screening
-> repair the P0 launch gates
-> validate one canonical publication path
-> launch only after controlled proof

V2/base Underwriting
-> become the eventual Full Underwriting Report
-> retain one canonical Source Truth and deterministic calculation layer
-> establish one explicit product identity and customer-surface contract
-> add governed safe-base fallback and published-with-limitations behavior
-> certify before paid launch

Premium
-> remain false and isolated
-> no customer activation until identity, completeness, receipt persistence,
   external certification, and fallback behavior are proven
```

## Exact next implementation boundary

Claude's final synthesis recommended:

```text
P0-A — authenticated identity and authorization boundary
```

This is an audit recommendation only. No P0-A implementation has started.

Before implementation, the Stage 11 synthesis and supporting Stage 1 through Stage 10 evidence must be uploaded and inspected. The first patch must remain narrowly bounded to authenticated identity and authorization. It must not broaden into worker, parser, renderer, Premium, billing, or publication refactoring.

## Current HOLD and restrictions

```text
AUDIT: COMPLETE
AUDIT BRANCH: ISOLATED / NOT MERGED
PRODUCTION PREMIUM ASSIGNMENT: FALSE
RETEST 39: NOT AUTHORIZED
P0-A IMPLEMENTATION: NOT STARTED
DEPLOYMENT FROM AUDIT: NONE
MIGRATION FROM AUDIT: NONE
CUSTOMER DATA CHANGE FROM AUDIT: NONE
```

Do not:

```text
merge the investigation branch to main
activate Premium
run another customer RETEST
reactivate legacy factual authority
weaken PDF fact-parity certification
allow AI to become factual authority
create another Boss or publication authority
patch only the latest reason code and declare launch readiness
start broad implementation before reviewing Stage 11 evidence
```

## Fresh-chat continuation point

Upload these four updated doctrine files together with Claude's canonical investigation index and all Stage 1 through Stage 11 evidence files.

First task:

```text
1. Inspect Stage 11 against the supporting stage evidence.
2. Return PASS or HOLD on the final audit recommendation.
3. Translate the exact 10 launch blockers into plain English.
4. Confirm whether P0-A is truly the correct first implementation phase.
5. Produce one tiny protected P0-A implementation prompt only.
```

No implementation, deployment, migration, environment change, production-data action, or live RETEST is authorized until that evidence review is complete.


## Updated product and launch checkpoint

The earlier roadmap that treated Premium Acquisition Underwriting as the immediate launch lane is superseded by the full-repository audit recommendation.

Current product sequence:

```text
1. Repair P0 launch blockers.
2. Validate and launch Screening first if the P0 gates pass.
3. Stabilize V2/base Underwriting as the eventual Full Underwriting Report.
4. Keep Premium false until its additive contract, completeness, and governed fallback are proven.
```

InvestorIQ is paused at an evidence-review boundary, not an implementation boundary. The next chat must review Claude's Stage 11 synthesis before authorizing P0-A.

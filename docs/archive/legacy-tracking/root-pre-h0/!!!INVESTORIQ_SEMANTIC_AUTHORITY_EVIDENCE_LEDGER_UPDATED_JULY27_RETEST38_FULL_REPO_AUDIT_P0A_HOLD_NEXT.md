...
## Semantic authority correction after full-repository investigation

The earlier conclusion that all legacy semantic authority was fully contained is no longer sufficient as an active launch statement. The repository-wide audit reported reachable legacy factual fallbacks, shared V1/V2 identity, competing surface authorities, customer-surface re-derivation, parser/classification drift, and archived semantic families that remain reachable.

Therefore:

```text
PROTECTED DOCTRINE:
Canonical Source Truth remains the only permitted factual authority.

CURRENT RUNTIME COMPLIANCE:
HOLD pending the audited P0 repairs and end-to-end proof.

PREMIUM AUTHORITY:
No customer authority while feature flag is false.

V2/BASE UNDERWRITING:
Recommended eventual Full Underwriting lane, but not launch-authorized.
```

No prior PASS receipt may override current contradictory repository evidence. Findings must be resolved by exact file and call-path proof, not by historical doctrine declarations alone.

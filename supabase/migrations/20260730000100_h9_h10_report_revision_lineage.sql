...
alter table if exists public.reports
  add constraint reports_revision_current_requires_published_check
  check (is_current_revision = false or status = 'published');
...
create unique index if not exists reports_one_current_published_revision_per_family_key
  ...
  where revision_family_key is not null and is_current_revision = true and status = 'published';
...
begin
  ...
  if NEW.is_current_revision = true and NEW.status <> 'published' then
    NEW.is_current_revision := false;
  end if;

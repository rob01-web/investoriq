...
check(html.includes("Current Debt-Inclusive Cost Coverage Ratio</td><td>63.6%"), "current GPR-basis debt-inclusive ratio is precisely labeled");
check(html.includes("Proposed Debt-Inclusive Cost Coverage Ratio</td><td>76.3%"), "proposed GPR-basis debt-inclusive ratio is precisely labeled");
check(html.includes("Debt-inclusive cost coverage basis"), "debt coverage basis label visible");
check(html.includes("(T12 operating expenses + annual debt service) / T12 gross potential rent"), "debt coverage formula visible");
check(html.includes("not a point-in-time physical occupancy threshold"), "debt ratio is not presented as physical occupancy");
check(html.includes("should not be compared directly with accepted occupancy or occupancy sensitivity cases"), "cross-basis comparison forbidden in visible copy");
check(!html.includes('data-iq-elite07-observation="occupancy_vs_debt_break_even"'), "invalid cross-basis occupancy observation suppressed");
check(!/percentage points (?:above|below) proposed debt-inclusive occupancy coverage point/i.test(html), "invalid occupancy-minus-GPR-ratio narrative absent");
...
check(!/\bBreak[- ]Even Occupancy\b/i.test(html), "debt customer surface does not collide with historical Break-Even Occupancy label");
check(!/Debt-Inclusive Occupancy Coverage Point/i.test(html), "legacy occupancy coverage-point label removed");

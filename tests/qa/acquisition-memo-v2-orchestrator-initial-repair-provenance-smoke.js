...
assert.match(orchestratorSource, /customerSurfaceModel/);
assert.match(orchestratorSource, /bossContract/);
...
orchestratorSource = orchestratorSource.replace(
  ...
  "  let initialRepairPlan = buildAcquisitionMemoV2BossRepairPlan({\n    customerSurfaceModelValidation: initialCustomerSurfaceModelValidation,\n  });\n  if (globalThis.__forceInitialRepairShouldRetry) {\n    initialRepairPlan = { ...initialRepairPlan, shouldRetry: true };\n  }\n"
);
orchestratorSource = orchestratorSource.replace(
  "    const initialRepairProvenanceRegressionViolations = buildRepairProvenanceRegressionViolations({\n",
  "    const initialRepairProvenanceRegressionViolations = globalThis.__forcedInitialRepairProvenanceRegressionViolations || buildRepairProvenanceRegressionViolations({\n"
);
...
globalThis.__forcedInitialRepairProvenanceRegressionViolations = [
  {
    code: "REPAIR_PROVENANCE_REGRESSION",
    severity: "critical",
    section: "unitMix",
    message: "forced regression",
  },
];
globalThis.__forceInitialRepairShouldRetry = true;
...
assert.equal(probe.renderAndValidateCalls, 0, output);
...
console.log("acquisition-memo-v2 orchestrator initial-repair provenance smoke PASS");

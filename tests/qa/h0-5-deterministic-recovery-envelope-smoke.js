...
const rentRoll = await recoverRentRollWithAI({
  text: "Unit 1 rents for $1,000.",
  tables: [],
  filename: "rent-roll.pdf",
});
assert.equal(rentRoll?.method, "ai_rent_roll_recovery_validated");

const acquisition = await recoverAcquisitionPurchaseAssumptionsWithAI({
...
assert.equal(acquisition?.method, "ai_support_doc_candidate_evidence_checked");

const t12 = await recoverT12WithAI({
...
assert.equal(t12?.method, "ai_t12_recovery_validated");

assert.equal(requests.length, 3);
assert.equal(requests[0].model, "gpt-4o-2024-08-06");
assert.equal(requests[0].temperature, 0);
assert.equal(requests[0].text.format.type, "json_schema");
assert.equal(requests[0].text.format.name, "rent_roll_recovery");

assert.equal(requests[1].model, "gpt-4o-2024-08-06");
assert.equal(requests[1].temperature, 0);
assert.equal(requests[1].text.format.type, "json_schema");
assert.equal(requests[1].text.format.name, "acquisition_support_doc_recovery");

assert.equal(requests[2].model, "gpt-4o-2024-08-06");
assert.equal(requests[2].temperature, 0);
assert.equal(requests[2].text.format.type, "json_schema");
assert.equal(requests[2].text.format.name, "t12_recovery");

console.log("h0-5 deterministic recovery envelope smoke PASS");

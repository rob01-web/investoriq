...
Write-Host ""
Write-Host "=== INVESTORIQ PHASE 2 FINAL LOCAL CERTIFICATION ===" -ForegroundColor Cyan
...
Write-Host ""
Write-Host "=== PATCHER SYNTAX ===" -ForegroundColor Cyan
node --check .\scripts\phase2-finalize-local.mjs
if ($LASTEXITCODE -ne 0) { throw "STOP: Phase 2 patcher syntax failed." }

Write-Host ""
Write-Host "=== APPLY GUARDED PHASE 2 INTEGRATION ===" -ForegroundColor Cyan
node .\scripts\phase2-finalize-local.mjs
if ($LASTEXITCODE -ne 0) { throw "STOP: Phase 2 guarded integration failed." }

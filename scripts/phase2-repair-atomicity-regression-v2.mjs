...
const status = git('status', '--porcelain=v1', '--untracked-files=all', '--', relativePath);
if (!status) {
  fail(`${relativePath} is not in the expected V4 modified state.`);
}
...
for (const marker of expectedMarkers) {
  if (!current.includes(marker)) {
    fail(`${relativePath} does not contain expected V4 marker: ${marker}`);
  }
}
if (current.includes("const p0c = fs.readFileSync(")) {
  fail(`${relativePath} unexpectedly reverted to the pre-Phase-2 authority.`);
}
...
console.log('Phase 2 atomicity regression V2 repair applied.');

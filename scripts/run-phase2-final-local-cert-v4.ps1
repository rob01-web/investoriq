...
Write-Host ""
Write-Host "=== INVESTORIQ PHASE 2 FINAL LOCAL CERTIFICATION V4 ===" -ForegroundColor Cyan
...
# V3 already produced these exact intended local changes before the stale atomicity regression stopped it.
$expectedPreservedChanges = @(
...
$unexpectedPre = @($preDirtyPaths | Where-Object { $expectedPreservedChanges -notcontains $_ })
$missingPreserved = @($expectedPreservedChanges | Where-Object { $preDirtyPaths -notcontains $_ })

if ($unexpectedPre.Count -gt 0 -or $missingPreserved.Count -gt 0) {
    Write-Host "STOP: Phase 2 V4 did not find the exact preserved V3 state." -ForegroundColor Red
    if ($missingPreserved.Count -gt 0) {
        Write-Host "Missing expected preserved files:" -ForegroundColor Yellow
        $missingPreserved
    }
...
Write-Host ""
Write-Host "=== REPAIR STALE ATOMICITY REGRESSION ===" -ForegroundColor Cyan
node --check .\scripts\phase2-repair-atomicity-regression.mjs
if ($LASTEXITCODE -ne 0) { throw "STOP: Atomicity regression repair script syntax failed." }
node .\scripts\phase2-repair-atomicity-regression.mjs
if ($LASTEXITCODE -ne 0) { throw "STOP: Atomicity regression repair failed." }

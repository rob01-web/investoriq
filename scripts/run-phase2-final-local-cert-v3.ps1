...
Write-Host ""
Write-Host "=== INVESTORIQ PHASE 2 FINAL LOCAL CERTIFICATION V3 ===" -ForegroundColor Cyan
...
$tempGeneratedPatcher = 'scripts/.phase2-finalize-local.generated.mjs'
...
$preDirtyPaths = @(
    $preDirtyLines | ForEach-Object { $_.Substring(3).Trim('"') }
)
$preAllowed = @($expectedIntegratedChanges + $tempGeneratedPatcher)
$unexpectedPre = @($preDirtyPaths | Where-Object { $preAllowed -notcontains $_ })
$missingIntegrated = @($expectedIntegratedChanges | Where-Object { $preDirtyPaths -notcontains $_ })

if ($unexpectedPre.Count -gt 0 -or $missingIntegrated.Count -gt 0) {
    Write-Host "STOP: Phase 2 V3 did not find the exact preserved V2 integration state." -ForegroundColor Red
    if ($missingIntegrated.Count -gt 0) {
        Write-Host "Missing expected integrated files:" -ForegroundColor Yellow
        $missingIntegrated
    }
...
if (Test-Path -LiteralPath $tempGeneratedPatcher) {
    Remove-Item -LiteralPath $tempGeneratedPatcher -Force
}

Write-Host ""
Write-Host "=== REPAIR STALE MANIFEST REGRESSION ===" -ForegroundColor Cyan
node --check .\scripts\phase2-repair-manifest-regression.mjs
if ($LASTEXITCODE -ne 0) { throw "STOP: Manifest regression repair script syntax failed." }
node .\scripts\phase2-repair-manifest-regression.mjs
if ($LASTEXITCODE -ne 0) { throw "STOP: Manifest regression repair failed." }

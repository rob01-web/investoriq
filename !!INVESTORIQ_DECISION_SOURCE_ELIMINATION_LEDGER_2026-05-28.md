...
3. Section eligibility / Data Coverage authority
...
| DS-030 | acquisition vs current debt | Underwriting | api/_lib/report-contract-qa.js | contamination checks | detect acquisition/current debt drift | rendered text + artifacts | blocking violations | QA-only | high | Financial Canonical Layer | convert to QA conformance only | B4 | closed | QA is canonical-first conformance/safety-net when canonical acquisition/current-debt state exists; rendered text/regex/table values are evidence-only; artifact-derived loan-term values cannot override canonical state; legacy artifact fallback is preserved only when canonical payload/state is absent; lender fee uses canonical `lender_fee_percent` when present; closing-cost note/zero check is fallback-only until canonical closing-cost fields exist; no hardcoded property names, filenames, report IDs, or one-off values. DS-021 not reopened; DS-029 not patched; DS-067 remains open as upstream parser/fallback ambiguity risk. |
